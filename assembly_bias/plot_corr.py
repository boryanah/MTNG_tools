...
for i, snapshot in enumerate(snapshots):
    ...
    for gal_type in gal_types:
        ...
        rat_mean = np.load(f"{gal_type:s}/corr_rat_shuff_mean_{n_gal}{drad_str}_{fp_dm}_{snapshot:d}.npy")
        rat_err = np.load(f"{gal_type:s}/corr_rat_shuff_err_{n_gal}{drad_str}_{fp_dm}_{snapshot:d}.npy")
        ...
        for i_pair in range(len(secondaries)):
            ...
            if fit_type == 'plane':
                if mode == 'bins':
                    rat_mean_this = np.load(f"{gal_type:s}/corr_rat_mean_{fun_type:s}_{secondary:s}_{tertiary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
                    rat_err_this = np.load(f"{gal_type:s}/corr_rat_err_{fun_type:s}_{secondary:s}_{tertiary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
                elif mode == 'all':
                    rat_mean_this = np.load(f"{gal_type:s}/corr_rat_mean_all_{fun_type:s}_{secondary:s}_{tertiary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
                    rat_err_this = np.load(f"{gal_type:s}/corr_rat_err_all_{fun_type:s}_{secondary:s}_{tertiary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
            else:
                if mode == 'bins':
                    rat_mean_this = np.load(f"{gal_type:s}/corr_rat_mean_{fun_type:s}_{secondary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
                    rat_err_this = np.load(f"{gal_type:s}/corr_rat_err_{fun_type:s}_{secondary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
                elif mode == 'all':
                    rat_mean_this = np.load(f"{gal_type:s}/corr_rat_mean_all_{fun_type:s}_{secondary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
                    rat_err_this = np.load(f"{gal_type:s}/corr_rat_err_all_{fun_type:s}_{secondary:s}{drad_str}_{n_gal}_{fp_dm:s}_{snapshot:d}.npy")
            ...
            if counter == 0:
                plt.text(x=0.1, y=0.83, s=label, transform=plt.gca().transAxes, fontsize=18)
...
plt.legend(ncol=1, fontsize=16, loc='lower right')

plt.subplot(nrow, len(secondaries)//nrow, len(secondaries)-1)
plt.legend(ncol=1, fontsize=16, loc='lower right')

plt.savefig(f"figs/corr_param_{n_gal}_{fp_dm:s}.png", bbox_inches='tight', pad_inches=0.)
#plt.close()
plt.show()
