...
for i, snapshot in enumerate(snapshots):
    ...
    for gal_type in gal_types:
        ...
        rat_shuff_mean = np.load(f"{gal_type:s}/corr_rat_shuff_mean_{n_gal}_fixocc_{fp_dm:s}_{snapshot_dm:d}.npy")
        rat_shuff_mean_np = np.load(f"{gal_type:s}/corr_rat_shuff_mean_{n_gal}_drad_fixocc_{fp_dm:s}_{snapshot_dm:d}.npy")
        ...
        #rat_shuff_mean_np = np.load(f"{gal_type:s}/ratl2_shuff_mean_{n_gal}_drad_{fp_dm:s}_{snapshot_dm:d}.npy") # xil2
        #rat_shuff_mean = np.load(f"{gal_type:s}/pair_rat_shuff_mean_{n_gal}_{fp_dm:s}_{snapshot_dm:d}.npy") # pair
        #rat_shuff_mean_np = np.load(f"{gal_type:s}/pair_rat_shuff_mean_{n_gal}_drad_{fp_dm:s}_{snapshot_dm:d}.npy") # pair
        #rbinc = np.load(f"{gal_type:s}/pair_rbinc.npy")
...
plt.plot([], [], color='black', ls='-', label=r'${\rm with \ associated}$')
plt.plot([], [], color='black', ls='--', label=r'${\rm w/o \ associated}$')
label = r"$n_{\rm gal} = %s \times 10^{-%s}$"%(p1, p2)
plt.text(0.6, 0.73, s=label, transform=plt.gca().transAxes)
