...
for i, snapshot in enumerate(snapshots):
    ...
    for gal_type in gal_types:
        ...
        pair_true_mean = np.load(f"{gal_type:s}/pair_mean_{n_gal}_{fp_dm:s}_{snapshot_dm:d}.npy")
        pair_shuff_mean = np.load(f"{gal_type:s}/pair_shuff_mean_{n_gal}_{fp_dm:s}_{snapshot_dm:d}.npy")
        rat_shuff_mean = np.load(f"{gal_type:s}/pair_rat_shuff_mean_{n_gal}_{fp_dm:s}_{snapshot_dm:d}.npy")
        rat_shuff_err = np.load(f"{gal_type:s}/pair_rat_shuff_err_{n_gal}_{fp_dm:s}_{snapshot_dm:d}.npy")
        rbinc = np.load(f"{gal_type:s}/pair_rbinc.npy")
        
        ax_scatter.errorbar(rbinc, pair_true_mean, capsize=4, color=hexcolors_bright[counter], ls='-', label=rf"${z_label}, \ {gal_label}$")
        ax_scatter.errorbar(rbinc, pair_shuff_mean, capsize=4, color=hexcolors_bright[counter], ls='--')
