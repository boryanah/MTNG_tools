...
GroupFirstSub_dm = np.load(tng_dir+f'data_dm/GroupFirstSub_dm_{snapshot_dm:d}.npy')
GroupNsubs_dm = np.load(tng_dir+f'data_dm/GroupNsubs_dm_{snapshot_dm:d}.npy')
...
GroupFirstSub_dm = GroupFirstSub_dm[i_sort]
GroupNsubs_dm = GroupNsubs_dm[i_sort]
...
GroupPos_dm %= Lbox
SubhaloPos_dm += np.array([Lbox/4., 0., 0.])
SubhaloPos_dm %= Lbox
...
#mbins = np.logspace(11, 13, 21) # TESTING!!!!!!!!!! super weird and off
#mbins = np.logspace(11, 12, 11) # TESTING!!!!!!!!!! super weird and off apparently because of the positions of the central wtf
#mbins = np.logspace(13, 14, 11) # TESTING!!!!!!!!!!  can't test cause need to add lower dudes as well
...
for i_pair in range(len(secondaries)):
    ...
    for i in range(len(mbins)-1)[::-1]: # needed to invert cause of abundance matching
        ...
        nhalo = np.sum(choice)
        if nhalo == 0: continue
        choice_dm = np.arange(sum_halo, sum_halo+nhalo, dtype=int) 
    ...
    if mode == 'all':
        ...
        choice_dm = np.arange(n_top, sum_halo, dtype=int) 
    ...
    for i in range(len(mbins)-1):
        ...
        first = GroupFirstSub_dm[choice]
        nsub = GroupNsubs_dm[choice]
        ...
        for j in range(len(cbins)-1):
            for k in range(len(ebins)-1):
                ...
                #ct = cts_sats_pred[rchoice]
                rcr = rcrit[rchoice]
                print("mean radius = ", i, j, k, np.mean(rcr))
                ind = index[rchoice]
                # index into radius
                ...
                for n in range(len(start)):
                    ...
                    poses = SubhaloPos_dm[start[n]:start[n]+ns[n]]
                    diff = poses - SubhaloPos_dm[start[n]]
                    diff[diff > Lbox/2.] -= Lbox
                    diff[diff < -Lbox/2.] += Lbox
                    ...
                    for m in range(len(radii)):
                        ...
                        if True:#dist_min > .4:
                            ...
                            pos_pred_sats[sum] = np.array([x, y, z]) + SubhaloPos_dm[start[n]]
                        else:
        ...
        print("number of galaxies not close to an existing subhalo = ", deba)
    ...
    print("number of satellites before addition of last crew = ", sum)
    sum += pos_new.shape[0]
    print("final number of satellites and expected = ", sum, pos_pred_sats.shape)
    pos_pred_sats = pos_pred_sats[:sum]
    ind_pred_sats = ind_pred_sats[:sum]
    
    # one question is what happens to the super massive dudes # TESTING!!!!!!!!!!!!!!!
    pos_new = SubhaloPos[index_cent[GrMcrit[SubhaloGrNr[index_cent]] > mbins[-1]]]
    ind_new = SubhaloGrNr[index_cent[GrMcrit[SubhaloGrNr[index_cent]] > mbins[-1]]]
    pos_pred_cent[:pos_new.shape[0]] = pos_new
    ind_pred_cent[:pos_new.shape[0]] = ind_new
